...
from config import AUDIO_EXTENSIONS, WATCH_DIR
...
def normalize_and_rename(file_path: Path) -> Path:
    ...
    if clean_name != original_name and clean_name:
        ...
        try:
            ...
            logger.warning(
                f"  [Rename Skipped] Could not rename '{original_name}': {e}"
            )
            return file_path
...
def register_file(
    conn, file_path: Path, retry_failed: bool = False
) -> tuple[bool, str]:
    ...
    try:
        ...
        cursor.execute(
            "SELECT file_hash, file_mtime, status FROM voice_notes WHERE file_path = ?",
            (str(file_path),),
        )
        row = cursor.fetchone()
...
def run_ingestion(retry_failed: bool = False) -> int:
        ...
        for root, _, files in os.walk(WATCH_DIR):
            for file in files:
                if (
                    ...
                    is_queued, action_type = register_file(
                        conn, clean_path, retry_failed=retry_failed
                    )
                    if is_queued:
                        queued_count += 1
    ...
    if queued_count > 0:
        logger.info(
            f"   └── ✅ Queued {queued_count} new/updated file(s) (Scanned: {total_scanned})."
        )
    else:
